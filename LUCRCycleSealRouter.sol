...
contract LUCRCycleSealRouter {
    ...
    event SealRouted(bytes32 indexed key, bytes32 sealHash, uint256 blockNum);
    ...
    function routeSeal(bytes32 key, bytes32 sealHash) external onlyGovernance {
        Target memory t = targets[key];
        require(t.enabled, "Target disabled");
        require(t.module != address(0), "Invalid target");

        (bool ok, ) = t.module.call(abi.encodeWithSignature("receiveSeal(bytes32)", sealHash));
        require(ok, "Seal routing failed");

        emit SealRouted(key, sealHash, block.number);
    }
}
